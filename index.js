...
const processObject = (obj, schema, item) => {
  ...
  for (const subItem in schema[item].properties) {
    ...
    subResult[subItem] = getTypeValue(obj[item]?.[subItem], subItemType, defaultValue);
  }
...
const processArray = (obj, schema, item) => {
  ...
  if (items.type === 'object') {
    const processedElements = obj[item]?.filter((element) => typeof element === 'object')
  ...
  if (['string', 'number', 'boolean', 'date', 'float', 'int'].includes(items.type)) {
    const processedElements = obj[item]?.filter((element) => typeof element === items.type)
